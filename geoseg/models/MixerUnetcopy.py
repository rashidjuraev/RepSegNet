...
class DepthWiseInceptionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
#######################################
class Rep_UNET_down(nn.Module):
    def __init__(self, num_classes):
        super(Rep_UNET_down, self).__init__()
        ...
        self.D_8_2 = nn.Sequential(DepthWiseInceptionBlock(128,64),
                                  nn.UpsamplingBilinear2d(scale_factor=2))
        ...
        self.D_4_2 = nn.Sequential(DepthWiseInceptionBlock(128,32),
                                  nn.UpsamplingBilinear2d(scale_factor=2))
        ...
        self.D_2_2 = nn.Sequential(DepthWiseInceptionBlock(64,32),
                                  nn.UpsamplingBilinear2d(scale_factor=2),)
        ...
        self.D_1_2 = nn.Sequential(DepthWiseInceptionBlock(64,8),
...
class DASKNET(nn.Module):
    def __init__(self, num_classes, c=64):
        ...
        self.c = c
        # Encoder Stage 1
        self.E_S1 = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=c, kernel_size=3, padding=1, stride=1),
                                  nn.BatchNorm2d(c),
                                  nn.ReLU(),
                                  DepthWiseInceptionBlock(c, c))
        ...
        self.E_S2 = nn.Sequential(DepthWiseInceptionBlock(c, c*2),
                                  DepthWiseInceptionBlock(c*2, c*2))

        # self.S2_path = ConvMixer_Path(128, 3)
        # self.S2_path_aggregate = Conv2d(128, 256, kernel_size=1)

        self.E_S3 = nn.Sequential(DepthWiseInceptionBlock(c*2, c*4),
                                  DepthWiseInceptionBlock(c*4, c*4))

        # self.S3_path = ConvMixer_Path(256, 2)
        # self.S3_path_aggregate = Conv2d(256, 512, kernel_size=1)

        self.E_S4 = nn.Sequential(DepthWiseInceptionBlock(c*4, c*8),
                                  DepthWiseInceptionBlock(c*8, c*8))

        # self.S4_path = ConvMixer_Path(512, 1)

        self.bridge = nn.Sequential(DepthWiseInceptionBlock(c*8, c*16),
                                    DepthWiseInceptionBlock(c*16, c*8))
        ...
        self.D_S4 = nn.Sequential(DepthWiseInceptionBlock(c*16, c*8),
                                    DepthWiseInceptionBlock(c*8, c*4))

        self.D_S3 = nn.Sequential(DepthWiseInceptionBlock(c*8,c*4),
                                    DepthWiseInceptionBlock(c*4, c*2))

        self.D_S2 = nn.Sequential(DepthWiseInceptionBlock(c*4, c*2),
                                    DepthWiseInceptionBlock(c*2, c))

        self.D_S1 = nn.Sequential(DepthWiseInceptionBlock(c*2, c),
                                    DepthWiseInceptionBlock(c, c))
